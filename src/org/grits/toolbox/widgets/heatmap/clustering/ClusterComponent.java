...
public class ClusterComponent implements Paintable {
	...
	@Override
	public void paint(Graphics g, int xDisplayOffset, int yDisplayOffset, double xDisplayFactor, double yDisplayFactor,
			boolean decorated, boolean horizontal) {
		paint(g, xDisplayOffset, yDisplayOffset, xDisplayFactor, yDisplayFactor, decorated, true);
    ...
    public void paint(Graphics g, int xDisplayOffset, int yDisplayOffset, double xDisplayFactor, double yDisplayFactor, boolean decorated, boolean horizontal, boolean primarySide) {
        int x1, y1, x2, y2;
       // FontMetrics fontMetrics =  FigureUtilities.getFontMetrics(labelFont);
      //  g.setBackgroundColor(XYGraphMediaFactory.getInstance().getColor(0, 0, 0));// set it to black to fill the oval
        if (!horizontal) {
	  ...
	  /*      if (cluster.isLeaf()) {
	        ...
	        if (decorated && cluster.getDistance() != null && !cluster.getDistance().isNaN() && cluster.getDistance().getDistance() > 0) {
	            ...
	            Dimension rect = FigureUtilities.getTextExtents(s, labelFont);
	            g.drawString(s, x1 - (int)rect.width , y1 - 2);
	        }*/
    ...
    public int getNameWidth(Graphics g, boolean includeNonLeafs) {
        ...
        if (includeNonLeafs || cluster.isLeaf()) {
            Dimension rect =  FigureUtilities.getTextExtents(cluster.getName(), labelFont);
